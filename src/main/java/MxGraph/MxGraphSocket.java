...
import Collaboration.GetHttpSessionConfigurator;

@ServerEndpoint(value = "/MxGraph/{groupID}", configurator=GetHttpSessionConfigurator.class) //类层次的注解，功能主要是将目前的类定义成一个websocket服务器端。注解的值是websocket的名字
public class MxGraphSocket
{
    private Session session=null;
    //初始化集合，用来存放每个客户端对应的服务器端的websocket对象
//    private static CopyOnWriteArraySet<MxGraphSocket> servers=new CopyOnWriteArraySet<>();
    private static final Map<String, Set<MxGraphSocket>> groups=new ConcurrentHashMap<>();
    ...
    public void onOpen(@PathParam("groupID") String groupID, Session session,EndpointConfig config)
    {
        ...
        System.out.println("连接已经建立,sessionID:"+session.getId()+"用户名："+httpSession.getAttribute("userName"));
        this.session=session;
        if (!groups.containsKey(groupID)){
            Set<MxGraphSocket> group=new HashSet<>();
            group.add(this);
...
//        System.out.println("客户端发送的数据:"+messageObject.getString("message"));
        ...
        if (messageType.equals("Message")){
            ...
            {
                //向客户端发送消息
                for (MxGraphSocket server:groups.get(groupID))
                {
...
//        members.remove(this.session.getId());
    ...
    private void ReGrant(String groupID){//重新设置演示者
        if (!requireLists.get(groupID).isEmpty()){//若请求队列的请求用户不为空
            ...
            boolean exist=false;
            for (MxGraphSocket server:groups.get(groupID)){//从群组的在线人员里找是否有申请者
