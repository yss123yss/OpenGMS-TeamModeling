...
@ServerEndpoint(value = "/LogicalModel/{groupID}", configurator=GetHttpSessionConfigurator.class) //类层次的注解，功能主要是将目前的类定义成一个websocket服务器端。注解的值是websocket的名字
public class LogicalModelSocket
{
    private Session session=null;
    //初始化集合，用来存放每个客户端对应的服务器端的websocket对象
//    private static CopyOnWriteArraySet<MxGraphSocket> servers=new CopyOnWriteArraySet<>();
    private static final Map<String, Set<LogicalModelSocket>> groups=new ConcurrentHashMap<>();
    ...
    public void onOpen(@PathParam("groupID") String groupID, Session session,EndpointConfig config)
    {
        ...
        if (!groups.containsKey(groupID)){
            Set<LogicalModelSocket> group=new HashSet<>();
            group.add(this);
...
//        members.remove(this.session.getId());
    ...
    private void ReGrant(String groupID){//重新设置演示者
        if (!requireLists.get(groupID).isEmpty()){//若请求队列的请求用户不为空
            ...
            boolean exist=false;
            for (LogicalModelSocket server:groups.get(groupID)){//从群组的在线人员里找是否有申请者
