...
Format.prototype.refresh = function()
{
	...
	if (graph.isSelectionEmpty())
	{
		...
		if((url.indexOf("index")<0)&&(url.indexOf("logical")<0)){
            mxUtils.write(label, "Conceptual Model");
		}else{
            if(url.indexOf("index")>=0){
                mxUtils.write(label, "Conceptual Modeling Scene");
            }else if(url.indexOf("logical")>=0){
                mxUtils.write(label, "Logical Modeling Scene");
            }else{
                mxUtils.write(label, "");
            }
		}
	...
	else
	{
        ...
        if (graph.getSelectionCount() > 1){
            ...
            showAttribute=true;
            mxUtils.write(label3, "System");
        }
        else  if (cell.isEdge()){
            showAttribute=true;
            mxUtils.write(label3, "Relation");
        }
        else if(ss.image||!cell.isEdge()){
            showAttribute=true;
            mxUtils.write(label3, "Element");
        }
        /////////////////////////////////////
        if (showAttribute) {
            label3.style.borderLeftWidth = '0px';
...
{
	var div = document.createElement('div');
	div.style.padding = '12px 18px 12px 18px';
...
ArrangePanel.prototype.init = function()
{
    ...
    //
    // var url = window.location.href;
    // if(url.indexOf("logical")>=0){
     //    var style = ss.style.image;
     //    if(style.indexOf("start")>=0){
     //        this.container.appendChild(this.addModelDesc(this.createPanel()));
	// 	}else if(style.indexOf("end")>=0){
     //        this.container.appendChild(this.addModelDesc(this.createPanel()));
	// 	}else if(style.indexOf("modelItem")>=0){
     //        this.container.appendChild(this.addModelDesc(this.createPanel()));
     //        this.container.appendChild(this.addItemRelated(this.createPanel(),"Model Item"));
	// 	}else if(style.indexOf("dataItem")>=0){
     //        this.container.appendChild(this.addModelDesc(this.createPanel()));
     //        this.container.appendChild(this.addItemRelated(this.createPanel(),"Data Item"));
     //    }else if(style.indexOf("dataItem")>=0){
     //        this.container.appendChild(this.addModelDesc(this.createPanel()));
     //        this.container.appendChild(this.addItemRelated(this.createPanel(),"Data Item"));
     //    }else if(style.indexOf("conditionItem")>=0){
     //        this.container.appendChild(this.addModelDesc(this.createPanel()));
     //        this.container.appendChild(this.addItemRelated(this.createPanel(),"Condition Item"));
     //    }else if(style.indexOf("operationItem")>=0){
     //        this.container.appendChild(this.addModelDesc(this.createPanel()));
     //        this.container.appendChild(this.addItemRelated(this.createPanel(),"Operation Item"));
     //    }else{
     //        this.container.appendChild(this.addModelDesc(this.createPanel()));
     //        this.container.appendChild(this.addItemRelated(this.createPanel(),"Dependency Item"));
	// 	}
    // }else{
     //    this.container.appendChild(this.addModelDesc(this.createPanel()));
     //    this.container.appendChild(this.addRelated(this.createPanel()));
	// }
    this.container.appendChild(this.addInfo(this.createPanel()));
};

/**
 * 
 */
ArrangePanel.prototype.addInfo = function(div)
{
	var ui = this.editorUi;
    var graph = ui.editor.graph;
	var ss = graph.getSelectionCell();
    ...
    if (graph.getSelectionCount() > 1) {
        ...
        textarea.style.border = 'solid 1px #d5d5d5';
		...
		var descTitle=document.createElement("div");
		descTitle.innerHTML="Related Elements";
        div.appendChild(descTitle);
        if (ss.source && ss.target) {
            ...
            if (source.name!=="undefined"||target.name!=="undefined") {
                ...
                li.innerHTML = "Source: " + source.name;
                var li2 = document.createElement("li");
                li2.innerHTML = "Target: " + target.name;
                ul.appendChild(li);
                ul.appendChild(li2);
            }
        }
		...
		var urlTitle=document.createElement("div");
		urlTitle.innerHTML="URL";
    	div.appendChild(urlTitle);
		if (ss.url) {
			urlText.value = ss.url;
		}
		urlText.setAttribute('type', 'text');
		urlText.style.fontSize = '12px';
		urlText.style.overflow = 'hidden';
		urlText.style.boxSizing = 'border-box';
		urlText.style.border = 'solid 1px #d5d5d5';
		urlText.style.borderRadius = '4px';
		urlText.style.width = '100%';
		urlText.style.outline = 'none';
		urlText.style.padding = '6px';
		urlText.style.display = 'block';
		urlText.style.marginBottom = "10px";
		div.appendChild(urlText);
    }
	var controlFolderDiv=document.createElement("div");
	var folderContentDiv=document.createElement("div");

	var addEmptyFolder=document.createElement("button");
	addEmptyFolder.innerHTML="+";
	addEmptyFolder.style.width="20px";
	addEmptyFolder.style.height="20px";
	addEmptyFolder.style.padding="0";
	addEmptyFolder.disabled=true;
	var that=this;
	addEmptyFolder.onclick=function(){
		if(localSelect.selectedIndex>0){
			that.createLocalizationPalette(localSelect.options[localSelect.selectedIndex].text,folderContentDiv);
		}
	}

	var localSpan=document.createElement("span");
	localSpan.innerHTML="Local:";
	controlFolderDiv.appendChild(localSpan);
	controlFolderDiv.style.margin="5px 0px 5px 0";
	var localSelect=document.createElement("select");
	var firstOption=document.createElement("option");
	firstOption.text="----- select -----";
	localSelect.appendChild(firstOption);
	for(var i=0;i<this.localArray.length;i++){
		var option=document.createElement('option');
		option.text=this.localArray[i];
		localSelect.appendChild(option);
	}
	localSelect.onchange=function(){
		if(localSelect.selectedIndex>0){
			addEmptyFolder.disabled=false;
		}
		else{
			addEmptyFolder.disabled=true;
		}
	}
	localSelect.style.width="125px";
	localSelect.style.margin="0 5.5px 0 5.5px";
	
	controlFolderDiv.appendChild(localSelect);
	controlFolderDiv.appendChild(addEmptyFolder);
	div.appendChild(controlFolderDiv);

	//show Localizations
	if(ss.concept){
		this.addLocalizationPalette(folderContentDiv,ss.concept);
	}
    ...
    button.onclick = function () {
		...
		ss.name = input.value;
		if(ss.concept){
			conceptItem.id=ss.concept;
			var index;
			for(var i=0;i<graph.conceptItemList.length;i++){
				if(graph.conceptItemList[i].id==ss.concept){
					index=i;
					break;
				}
			}
			graph.conceptItemList.splice(index,1);
		}else{
			conceptItem.id=that.createUUID();
			ss.concept=conceptItem.id;
		}
		conceptItem.wkname=input.value;
		conceptItem.url=urlText.value;
		ss.url=conceptItem.url;
		var Localizations=[];
		for(var i=0;i<folderContentDiv.childElementCount;i++){
			var Localization={};
			var LocalFolderDiv=folderContentDiv.childNodes[i];
			var localTitle= LocalFolderDiv.childNodes[0];
			var local=localTitle.innerText.replace(/\s/g,'').substring(1);
			Localization.local=local;
			var localNameContent=LocalFolderDiv.childNodes[1].childNodes[0];
			var localName=localNameContent.childNodes[1].value;
			Localization.name=localName;
			var localDescContent=LocalFolderDiv.childNodes[1].childNodes[1];
			var localDesc=localDescContent.childNodes[1].value;
			Localization.description=localDesc;
			Localizations.push(Localization);
		}
		conceptItem.localizations=Localizations;
		graph.conceptItemList.push(conceptItem)
		// console.log(graph.conceptItemList);
        graph.cellLabelChanged(ss, input.value, false);
    };
 ...
 * addFoldingHandler
 */
ArrangePanel.prototype.createLocalizationPalette=function(title,div){
	var Localizations=[];
	var Localization={"local":title,"name":"","description":""};
	Localizations.push(Localization);
	var that=this;
	div.appendChild(that.addPalette(Localizations[0]));
}

ArrangePanel.prototype.removeLocalizationPalette=function(div){
	var eltTitle=div.parentNode;
	eltTitle.parentNode.remove();
}

ArrangePanel.prototype.addLocalizationPalette=function(div,conceptID){
	this.addPaletteFunctions(div,conceptID);
}

ArrangePanel.prototype.addPaletteFunctions = function(div, conceptID)
 {
	 var conceptItem={};
	 var graph=this.editorUi.editor.graph;
	 for(var i=0;i<graph.conceptItemList.length;i++){
		 if(graph.conceptItemList[i].id==conceptID){
			conceptItem=graph.conceptItemList[i];
			break;
		 }
	 }
	 if(conceptItem.localizations){
		for(var i=0;i<conceptItem.localizations.length;i++){
			div.appendChild(this.addPalette(conceptItem.localizations[i]));
		 }
	 }
};

ArrangePanel.prototype.addPalette = function(Localization)
{
	var div=document.createElement("div");

    var local = this.createTitle(Localization.local);
	...
	if(Localization[1]!=""){
		nameContent.value=Localization.name;
	}
	...
	if(Localization[2]!=""){
		descContent.innerHTML=Localization.description;
	}
	descDiv.appendChild(descTitle);
	descDiv.appendChild(descContent);
	divFold.appendChild(descDiv);descContent
	...
	this.addFoldingHandler(local,divFold);

    return div;
};

ArrangePanel.prototype.addFoldingHandler = function(title, content, funct)
{
...
ArrangePanel.prototype.collapsedImage = (!mxClient.IS_SVG) ? IMAGE_PATH + '/collapsed.gif' : 'data:image/gif;base64,';

ArrangePanel.prototype.expandedImage = (!mxClient.IS_SVG) ? IMAGE_PATH + '/expanded.gif' : 'data:image/gif;base64,';

ArrangePanel.prototype.localArray=['ZH-CN','ZH-TW','ZH-HK','EN-HK','EN-US','EN-GB','EN-WW','EN-CA','EN-AU','EN-IE','EN-FI','FI-FI','EN-DK','DA-DK','EN-IL','HE-IL','EN-ZA','EN-IN','EN-NO','EN-SG','EN-NZ','EN-ID','EN-PH','EN-TH','EN-MY','EN-XA','KO-KR','JA-JP','NL-NL','NL-BE','PT-PT','PT-BR','FR-FR','FR-LU','FR-CH','FR-BE','FR-CA','ES-LA','ES-ES','ES-AR','ES-US','ES-MX','ES-CO','ES-PR','DE-DE','DE-AT','DE-CH','RU-RU','IT-IT','EL-GR','NO-NO','HU-HU','TR-TR','CS-CZ','SL-SL','PL-PL','SV-SE'];

ArrangePanel.prototype.createTitle = function(label)
{
	...
	elt.style.padding="6px 0px 6px 14px";
	elt.style.background="#eeeeee";
	elt.style.cursor="pointer";
	elt.style.borderBottom="1px solid #e5e5e5";
	...
	removeFolder.onclick=function(){
		that.removeLocalizationPalette(this);
	}
...
ArrangePanel.prototype.createUUID=function () {
...
ArrangePanel.prototype.addRelated = function (div) {

    div.appendChild(this.createTitle("Name"));
    var input = document.createElement('input');
// input.setAttribute('placeholder', "Name");
    ...
    var cell = graph.getSelectionCell();
        if(graph.getSelectionCount()>1){
            div.appendChild(this.createTitle("Description"));
            ...
            // div.appendChild(this.createTitle("Related System"));
            // div.appendChild(ul);
        }else if(cell.isEdge()) {

            div.appendChild(this.createTitle("Related elements"));
            div.appendChild(ul);
        }
        else if(ss.image||!cell.isEdge()) {
            div.appendChild(this.createTitle("Description"));
...
ArrangePanel.prototype.addItemRelated = function (div,title) {
    var ss = this.format.getSelectionState();
    ...
    div.appendChild(this.createTitle(title));
    div.appendChild(ul);
...
StyleFormatPanel.prototype.addStroke = function(container)
{
	...
	{
		addItem(menu, 70, 'solid', [mxConstants.STYLE_DASHED, mxConstants.STYLE_DASH_PATTERN], [null, null]).setAttribute('title', mxResources.get('solid'));
		addItem(menu, 70, 'dashed', [mxConstants.STYLE_DASHED, mxConstants.STYLE_DASH_PATTERN], ['1', null]).setAttribute('title', mxResources.get('dashed'));
		addItem(menu, 70, 'dotted', [mxConstants.STYLE_DASHED, mxConstants.STYLE_DASH_PATTERN], ['1', '1 1']).setAttribute('title', mxResources.get('dotted') + ' (1)');
		addItem(menu, 70, 'dotted', [mxConstants.STYLE_DASHED, mxConstants.STYLE_DASH_PATTERN], ['1', '1 2']).setAttribute('title', mxResources.get('dotted') + ' (2)');
		addItem(menu, 70, 'dotted', [mxConstants.STYLE_DASHED, mxConstants.STYLE_DASH_PATTERN], ['1', '1 4']).setAttribute('title', mxResources.get('dotted') + ' (3)');
	}));
...
DiagramFormatPanel.prototype.init = function()
{
    ...
    this.container.appendChild(this.addBasic(this.createPanel()));
 ...
 * yiyou edit
 */
DiagramFormatPanel.prototype.addBasic = function(div){
    var ui = this.editorUi;
    var editor = ui.editor;
    var graph = editor.graph;
	//添加Task命名文本框
    ...
    // input.setAttribute('placeholder', "Name");
    input.setAttribute('type', 'text');
    input.setAttribute('id', 'graphName');
    input.style.fontSize = '12px';
    input.style.overflow = 'hidden';
    input.style.boxSizing = 'border-box';
    input.style.border = 'solid 1px #d5d5d5';
    ...
    input.style.display = 'block';
    input.style.marginBottom = "10px";
    if (window.taskName!=="undefined"&&window.taskName!==""&&window.taskName!=null) {
    	input.value=window.taskName;
	}
    div.appendChild(input);
    ...
    // textarea.setAttribute('placeholder', "Description");
    textarea.setAttribute('type', 'text');
    textarea.setAttribute('id', 'graphDescription');
    textarea.style.fontSize = '12px';
    textarea.style.fontFamily = 'Arial';
    textarea.style.overflow = 'hidden';
    textarea.style.boxSizing = 'border-box';
    ...
    textarea.style.marginTop = '5px';
    textarea.rows = 15;
    if (window.taskDescription&&window.taskDescription!==""&&window.taskDescription!=null) {
        textarea.value=window.taskDescription;
    }
    textarea.onclick = function(){
    	this.focus();
	};
    ...
    // saveBt.setAttribute('id','graphSave');
    // saveBt.style.height='22px';
    // saveBt.style.width='56px';
    // saveBt.style.cursor='pointer';
    // saveBt.style.marginLeft='5px';
    // saveBt.style.marginRight='5px';
    // saveBt.innerText='Save';
    // saveBt.onclick=function (ev) { GraphSave(); };
	// //Task读取按钮
    // var readBt=document.createElement('button');
    // // readBt.setAttribute('data-toggle','modal');
    // // readBt.setAttribute('data-target','#myModal');
    // readBt.style.height='22px';
    // readBt.style.width='56px';
    // readBt.style.cursor='pointer';
    // readBt.style.marginLeft='5px';
    // readBt.style.marginRight='5px';
    // readBt.innerText='Read';
    // readBt.onclick=function (ev) { TasksModel(); };
    // //添加按钮
    // buttonDiv.appendChild(saveBt);
    // buttonDiv.appendChild(readBt);
    // div.appendChild(buttonDiv);
    return div;
};
