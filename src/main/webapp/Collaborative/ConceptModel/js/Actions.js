...
Actions.prototype.init = function()
{
    ...
    {
        var dlg = new ConceptTasksReadDialog(ui);
        ui.showDialog(dlg.container, 500, 600, true, false);
    ...
    this.addAction('UpdateTask', function()
    {
    	if(window.taskID==null||window.taskID==="undefined"||window.taskID===""){
            ...
            dataJSON["Model"]="Concept";
            dataJSON["Type"]="Update";
            dataJSON["ProjectID"]=window.taskID;
            dataJSON["GraphXML"]=graphContentXML;
			dataJSON["ConceptXML"]=ConceptualScene;
            dataJSON["TaskName"]=$('#graphName').val();
            dataJSON["Description"]=$('#graphDescription').val();
            $.ajax({
                type: "POST",
    ...
    //ExportImage
    this.addAction('ExportImage', function()
    {
		// function createUUID () {
		...
		// 		let id=createUUID();
		// 		node.children[i].UID=id;
		// 		if(node.children[i].style==="group"){
		// 			setUID(node.children[i]);
		// 		}
		// 	}
		// }
		...
		// var SystemCollection=XMLDoc.createElement("SystemCollection");
		// var ElementCollection=XMLDoc.createElement("ElementCollection");
		// var RelationCollection=XMLDoc.createElement("RelationCollection");
		// backOrder(node);
		// function backOrder(node){
		// 	for(let i=0;i<node.children.length;i++){
		// 		if(node.children[i].style==="group"){
		// 			let System=XMLDoc.createElement("System");
		// 			System.setAttribute("id",node.children[i].UID);
		// 			System.setAttribute("name",node.children[i].name);
		// 			System.setAttribute("concept",node.children[i].concept);
		// 			System.setAttribute("graph",node.children[i].id);
		// 			let systemElementCollection=XMLDoc.createElement("ElementCollection");
		// 			let systemRelationCollection=XMLDoc.createElement("RelationCollection");
		// 			let systemSubsystems=XMLDoc.createElement("Subsystems");
		// 			for(let j=0;j<node.children[i].children.length;j++){
		// 				if(node.children[i].children[j].style==="group"){
		// 					let SystemRef=XMLDoc.createElement("SystemRef");
		// 					SystemRef.setAttribute("id",node.children[i].children[j].UID);
		// 					systemSubsystems.appendChild(SystemRef);
		// 				}
		// 				else if(node.children[i].children[j].isEdge()){
		// 					let RelationRef=XMLDoc.createElement("RelationRef");
		// 					RelationRef.setAttribute("id",node.children[i].children[j].UID);
		// 					systemRelationCollection.appendChild(RelationRef);
		// 				}
		// 				else if(!node.children[i].children[j].isEdge()){
		// 					let ElementRef=XMLDoc.createElement("ElementRef");
		// 					ElementRef.setAttribute("id",node.children[i].children[j].UID);
		// 					systemElementCollection.appendChild(ElementRef);
		// 				}
		// 				System.appendChild(systemElementCollection);
		// 				System.appendChild(systemRelationCollection);
		// 				System.appendChild(systemSubsystems);
		// 			}
		// 			backOrder(node.children[i]);
		// 			SystemCollection.appendChild(System);
		// 		}
		// 		else if(node.children[i].isEdge()){
		// 			let Relation =XMLDoc.createElement("Relation");
		// 			Relation.setAttribute("id",node.children[i].UID);
		// 			Relation.setAttribute("name",node.children[i].name);
		// 			Relation.setAttribute("concept",node.children[i].concept);
		// 			Relation.setAttribute("graph",node.children[i].id);
		// 			let sourceSystemRef=XMLDoc.createElement("SystemRef");
		// 			if(node.children[i].source!=null){
		// 				sourceSystemRef.setAttribute("id",node.children[i].source.UID);
		// 			}
		// 			else{
		// 				sourceSystemRef.setAttribute("id","");
		// 			}
		// 			let targetSystemRef=XMLDoc.createElement("SystemRef");
		// 			if(node.children[i].target!=null){
		// 				targetSystemRef.setAttribute("id",node.children[i].target.UID);
		// 			}
		// 			else{
		// 				targetSystemRef.setAttribute("id","");
		// 			}
		// 			Relation.appendChild(sourceSystemRef);
		// 			Relation.appendChild(targetSystemRef);
		// 			RelationCollection.appendChild(Relation);
		// 		}
		// 		else if(!node.children[i].isEdge()){
		// 			let eElement=XMLDoc.createElement("Element");
		// 			eElement.setAttribute("id",node.children[i].UID);
		// 			eElement.setAttribute("name",node.children[i].name);
		// 			eElement.setAttribute("concept",node.children[i].concept);
		// 			eElement.setAttribute("graph",node.children[i].id);
		// 			ElementCollection.appendChild(eElement);
		// 		}
		// 	}
		// }
		// //ConceptItem
		// var ConceptItemList=XMLDoc.createElement("ConceptItemList");
		// for(var i=0;i<graph.conceptItemList.length;i++){
		// 	var conceptItem=XMLDoc.createElement("ConceptItem");
		// 	conceptItem.setAttribute("id",graph.conceptItemList[i].id);
		// 	conceptItem.setAttribute("wkname",graph.conceptItemList[i].wkname);
		// 	conceptItem.setAttribute("url",graph.conceptItemList[i].url);
		// 	for(var j=0;j<graph.conceptItemList[i].localizations.length;j++){
		// 		var localization=XMLDoc.createElement("Localization");
		// 		localization.setAttribute("local",graph.conceptItemList[i].localizations[j].local);
		// 		localization.setAttribute("name",graph.conceptItemList[i].localizations[j].name);
		// 		localization.setAttribute("description",graph.conceptItemList[i].localizations[j].description);
		// 		conceptItem.appendChild(localization);
		// 	}
		// 	ConceptItemList.appendChild(conceptItem);
		// }

		// var ConceptualScene=XMLDoc.createElement("ConceptualScene");
		// var id=createUUID();
		// ConceptualScene.setAttribute("id",id);
		// ConceptualScene.setAttribute("name","");
		// ConceptualScene.setAttribute("version","1");
		// ConceptualScene.appendChild(ElementCollection);
		// ConceptualScene.appendChild(SystemCollection);
		// ConceptualScene.appendChild(RelationCollection);
		// ConceptualScene.appendChild(ConceptItemList);
		// console.log(ConceptualScene);
		// var ConceptualSceneXMLStr=mxUtils.getXml(ConceptualScene);
		// console.log(ConceptualSceneXMLStr);

		// var encoder = new mxCodec();
        // var node = encoder.encode(graph.getModel());
        // var Integration = mxUtils.getXml(node);
        // var data = JSON.stringify(Integration);
        // var blob = new Blob([Integration], {type: "text/plain;charset=utf-8"}),
		// fileName = 'logical.mdl';
		/*------------------------------------------------------------------------------------------*/

		var ConceptualSceneXMLStr=graph.getConceptualSceneStr();
		var blob = new Blob([ConceptualSceneXMLStr], {type: "text/plain;charset=utf-8"}),
		fileName = 'concept.mdl';
		...
		// var Str='<ConceptualScene id="7014115d-2054-4c5e-99ed-ed786574cd32" name="" version="1"><ElementCollection><Element id="17bb76b7-dacc-4767-9b5c-71caac4b9756" name="asdasd" concept="bdb84784-62ef-4c59-ae82-2e8551829a7b" graph="4"/><Element id="674b3b05-82c6-440f-85a6-55151ff0dbab" name="sun" concept="f606b3bb-cfb4-49f8-bf7e-349c03aff834" graph="2"/><Element id="e4457eb3-4d3e-4fb3-89da-65e6d96b42d6" name="cloud" concept="95af83eb-07cb-4d24-a9c3-3062d9e0d52b" graph="3"/></ElementCollection><SystemCollection><System id="b8d7a9aa-0683-4aa7-a4c1-5a9cdb6c7ba2" name="system" concept="a36e092f-0c41-4ece-9635-d7e961cda121" graph="6"><ElementCollection><ElementRef id="674b3b05-82c6-440f-85a6-55151ff0dbab"/><ElementRef id="e4457eb3-4d3e-4fb3-89da-65e6d96b42d6"/></ElementCollection><RelationCollection><RelationRef id="4da80d54-9fa9-40cf-a963-7fb29678d1e8"/></RelationCollection><Subsystems/></System></SystemCollection><RelationCollection><Relation id="4da80d54-9fa9-40cf-a963-7fb29678d1e8" name="asdsadasd" concept="10d15f6a-07ec-482a-9b2f-e7d451c20547" graph="5"><SystemRef id="674b3b05-82c6-440f-85a6-55151ff0dbab"/><SystemRef id="e4457eb3-4d3e-4fb3-89da-65e6d96b42d6"/></Relation></RelationCollection><ConceptItemList><ConceptItem id="f606b3bb-cfb4-49f8-bf7e-349c03aff834" wkname="sun" url="asdsadas"><Localizations><Localization local="ZH-CN" name="asdsadas" description="sadsadsadsadas"/></Localizations></ConceptItem><ConceptItem id="95af83eb-07cb-4d24-a9c3-3062d9e0d52b" wkname="cloud" url="sadd"><Localizations><Localization local="ZH-CN" name="dasdas" description="sadsadsa"/><Localization local="ZH-CN" name="asdsad" description="sadsadasdsa"/></Localizations></ConceptItem><ConceptItem id="bdb84784-62ef-4c59-ae82-2e8551829a7b" wkname="asdasd" url="asdsadsadsa"><Localizations><Localization local="EN-HK" name="asdsad" description="asdsadsadsad"/></Localizations></ConceptItem><ConceptItem id="10d15f6a-07ec-482a-9b2f-e7d451c20547" wkname="asdsadasd" url=""><Localizations/></ConceptItem><ConceptItem id="a36e092f-0c41-4ece-9635-d7e961cda121" wkname="system" url="sadsadsa"><Localizations><Localization local="ZH-CN" name="asdsadsa" description="dsadsadasd"/></Localizations></ConceptItem></ConceptItemList></ConceptualScene>';
		// var regexp=/<ConceptItemList>([\S\s]*)<\/ConceptItemList>/;
		// var conceptItemListStr=Str.match(regexp)[0];
		// var doc = mxUtils.parseXml(conceptItemListStr).children[0];

		// var conceptItemList=[];
		// for(let i=0;i<doc.children.length;i++){
		// 	let conceptItem={};
		// 	let id=doc.children[i].getAttribute("id");
		// 	conceptItem.id=id;
		// 	let wkname=doc.children[i].getAttribute("wkname");
		// 	conceptItem.wkname=wkname;
		// 	let url=doc.children[i].getAttribute("url");
		// 	conceptItem.url=url;
		// 	let localizations=[];
		// 	for(let j=0;j<doc.children[i].children[0].children.length;j++){
		// 		let localization={};
		// 		let local=doc.children[i].children[0].children[j].getAttribute("local");
		// 		localization.local=local;
		// 		let name=doc.children[i].children[0].children[j].getAttribute("name");
		// 		localization.name=name;
		// 		let description=doc.children[i].children[0].children[j].getAttribute("description");
		// 		localization.description=description;
		// 		localizations.push(localization);
		// 	}
		// 	conceptItem.localizations=localizations;
		// 	conceptItemList.push(conceptItem);
		// }
		// graph.conceptItemList=conceptItemList;

		// graph.setConceptItemList(Str);
