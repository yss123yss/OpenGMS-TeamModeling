...
Menus.prototype.createPopupMenu = function(menu, cell, evt)
{
	...
	if (!graph.isSelectionEmpty())
	{
		...
		if (state != null)
		{
            ...
            var  type =  state.style.shape.substr(18);
			if((type==="modelitem")&&($("#viewPanel").val()==="Integration")){
                this.addMenuItems(menu, ['toModel'], null, evt);
			}
            if((type==="modelitem")&&($("#viewPanel").val()==="Condition")){
                this.addMenuItems(menu, ['toModel'], null, evt);
            }

            if((type==="condition")&&($("#viewPanel").val()==="Integration")){
                this.addMenuItems(menu, ['toCondition'], null, evt);
            }

            if((type==="dataitem")&&($("#viewPanel").val()!="Data")){
                this.addMenuItems(menu, ['toDataDependency'], null, evt);
            }
