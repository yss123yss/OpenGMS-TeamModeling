...
Actions.prototype.init = function()
{
	...
	this.addAction('toModel',function () {
        var cell = graph.getSelectionCell();
        if(cell.uid!=undefined){
				...
				if(select.value == "Model"){
					...
					for(let i=0;i<graph.mxgraphList.length;i++){
						if(graph.mxgraphList[i].uid==cell.uid){
							graph.mxgraphList[i].graphXML=ModelDependency;
							break;
						}
					}
				}
			...
			if(!exist){
				var doc = mxUtils.parseXml(emptyGraph);
           	 	var dec = new mxCodec(doc);
            	dec.decode(doc.documentElement, graph.getModel());
                cell.geometry.x = 350;
                cell.geometry.y = 250;
                graph.setSelectionCells(graph.importCells([cell], 0, 0));
                var node2 = dec.encode(graph.getModel());
				var modelGraphXML = mxUtils.getXml(node2);
				var newGraphXML={};
				newGraphXML.uid=cell.uid;
				newGraphXML.graphXML=modelGraphXML;
				graph.mxgraphList.push(newGraphXML);
			}

		}else{
			alert("Please Set Model Item Name")
		}
    });

	this.addAction('toCondition',function () {
        var cell = graph.getSelectionCell();
        if(cell.uid!=undefined){
            for(let i=0;i<graph.mxgraphList.length;i++){
				if(graph.mxgraphList[i].uid==$("#viewPanel option:selected").attr("currentUID")){
            ...
            var option = document.createElement("option");
            option.innerHTML = "Condition Dependency View";
            option.value = "Condition";
			option.setAttribute("currentUID",cell.uid);
            var select = document.getElementById("viewPanel");
            select.appendChild(option);
            $("#viewPanel").val("Condition");

            select.onclick = function () {
                if(select.value == "Condition"){
                    var encoder = new mxCodec();
                    var node = encoder.encode(graph.getModel());
                    ConditionDependency = mxUtils.getXml(node);
					for(let i=0;i<graph.mxgraphList.length;i++){
						if(graph.mxgraphList[i].uid==cell.uid){
							graph.mxgraphList[i].graphXML=ConditionDependency;
							break;
						}
					}
                }
			...
			if(!exist){
                ...
                graph.setSelectionCells(graph.importCells([cell], 0, 0));
                var node2 = dec.encode(graph.getModel());
				var conditionGraphXML = mxUtils.getXml(node2);
				var newGraphXML={};
				newGraphXML.uid=cell.uid;
				newGraphXML.graphXML=conditionGraphXML;
				graph.mxgraphList.push(newGraphXML);
			}
        }else{
            alert("Please Set Condition Item Name")
        }
    });

    this.addAction('toDataDependency',function () {
        var cell = graph.getSelectionCell();
        if(cell.uid!=undefined){
            for(let i=0;i<graph.mxgraphList.length;i++){
				if(graph.mxgraphList[i].uid==$("#viewPanel option:selected").attr("currentUID")){
            ...
            var option = document.createElement("option");
            option.innerHTML = "Data Dependency View";
            option.value = "Data";
			option.setAttribute("currentUID",cell.uid);
            var select = document.getElementById("viewPanel");
            select.appendChild(option);
            ...
            select.onclick = function () {
                if(select.value == "Data"){
                    var encoder = new mxCodec();
                    var node = encoder.encode(graph.getModel());
                    DataDependency = mxUtils.getXml(node);
                    for(let i=0;i<graph.mxgraphList.length;i++){
						if(graph.mxgraphList[i].uid==cell.uid){
							graph.mxgraphList[i].graphXML=DataDependency;
							break;
						}
					}
                }
            ...
            alert("Please Set Data Item Name")
        }
    });
