...
/**
 * Get the XML of mxgraph list in type string.
 */
Graph.prototype.getMxgraphListXMLStr=function(){
	var XMLDoc = mxUtils.createXmlDocument();
	var mxgraphList=this.mxgraphList;
	var mxgraphListXML=XMLDoc.createElement("MxGraphCollection");
	mxgraphListXML.setAttribute("name","");
	mxgraphListXML.setAttribute("description","");
	mxgraphListXML.setAttribute("version","1");
	for(let i=0;i<mxgraphList.length;i++){
		let mxgraph=XMLDoc.createElement("MxGraph");
		mxgraph.setAttribute("uid",mxgraphList[i].uid);
		mxgraph.setAttribute("graphXML",mxgraphList[i].graphXML);
		mxgraphListXML.appendChild(mxgraph);
	}
	console.log(mxgraphListXML);
	var mxgraphListXMLStr=mxUtils.getXml(mxgraphListXML);
	return mxgraphListXMLStr;
}

/**
 * Get the XML of logical scene list in type string.
 */
Graph.prototype.getLogicalSceneXMLStr=function(){
	function createUUID () {
	...
	var XMLDoc=mxUtils.createXmlDocument();
	var logicalSceneXML=XMLDoc.createElement("LogicalScene");
	var id=createUUID();
	logicalSceneXML.setAttribute("uid",id);
	logicalSceneXML.setAttribute("name","");
	logicalSceneXML.setAttribute("version","1");

	var modelItemsXML=XMLDoc.createElement("ModelCollection");
	var dataItemsXML=XMLDoc.createElement("DataCollection");
	var conditionItemsXML=XMLDoc.createElement("ConditionCollection");
	var operationsXML=XMLDoc.createElement("OperationCollection");
	var dependenciesXML=XMLDoc.createElement("DependencyCollection");
	
	var modelItems=this.logicalScene.modelItems;
	var dataItems=this.logicalScene.dataItems;
	var conditionItems=this.logicalScene.conditionItems;
	var operations=this.logicalScene.operations;
	var dependencies=this.logicalScene.dependencies;
	for(let i=0;i<modelItems.length;i++){
		var modelItem=XMLDoc.createElement("ModelItem");
		modelItem.setAttribute("uid",modelItems[i].uid);
		modelItem.setAttribute("name",modelItems[i].name);
		modelItem.setAttribute("description",modelItems[i].description);
		modelItemsXML.appendChild(modelItem);
	}
	for(let i=0;i<dataItems.length;i++){
		var dataItem=XMLDoc.createElement("DataItem");
		dataItem.setAttribute("uid",dataItems[i].uid);
		dataItem.setAttribute("name",dataItems[i].name);
		dataItem.setAttribute("description",dataItems[i].description);
		dataItemsXML.appendChild(dataItem);
	}for(let i=0;i<conditionItems.length;i++){
		var conditionItem=XMLDoc.createElement("ConditionItem");
		conditionItem.setAttribute("uid",conditionItems[i].uid);
		conditionItem.setAttribute("name",conditionItems[i].name);
		conditionItem.setAttribute("description",conditionItems[i].description);
		for(let j=0;j<dependencies.length;j++){
			if(dependencies[j].to==conditionItems[i].uid){
				var relatedDataItem=XMLDoc.createElement("RelatedDataItem");
				relatedDataItem.setAttribute("dataId",dependencies[j].from);
				conditionItem.appendChild(relatedDataItem);
			}
			if(dependencies[j].from==conditionItems[i].uid){
				var branch=XMLDoc.createElement("Branch");
				branch.setAttribute("id",dependencies[j].uid);
				branch.setAttribute("name",dependencies[j].name);
				branch.setAttribute("description",dependencies[j].description);
				branch.setAttribute(dependencies[j].totype+"Id",dependencies[j].to);
				conditionItem.appendChild(branch);
			}
		}
		conditionItemsXML.appendChild(conditionItem);
	}for(let i=0;i<operations.length;i++){
		var operation=XMLDoc.createElement("OperationItem");
		operation.setAttribute("uid",operations[i].uid);
		operation.setAttribute("name",operations[i].name);
		operation.setAttribute("description",operations[i].description);
		for(let j=0;j<dependencies.length;j++){
			if(dependencies[j].to==operations[i].uid){
				var input=XMLDoc.createElement("Input");
				input.setAttribute("dataRef",dependencies[j].from);
				operation.appendChild(input);
			}
			if(dependencies[j].from==operations[i].uid){
				var output=XMLDoc.createElement("Output");
				output.setAttribute("dataRef",dependencies[j].to);
				operation.appendChild(output);
			}
		}
		operationsXML.appendChild(operation);
	}for(let i=0;i<dependencies.length;i++){
		var dependency=XMLDoc.createElement("DependencyItem");
		dependency.setAttribute("uid",dependencies[i].uid);
		dependency.setAttribute("name",dependencies[i].name);
		dependency.setAttribute("description",dependencies[i].description);
		var from=XMLDoc.createElement("From");
		from.setAttribute("type",dependencies[i].fromtype);
		from.setAttribute("idRef",dependencies[i].from);
		var to=XMLDoc.createElement("To");
		to.setAttribute("type",dependencies[i].totype);
		to.setAttribute("idRef",dependencies[i].to);
		dependency.appendChild(from);
		dependency.appendChild(to);
		dependenciesXML.appendChild(dependency);
	}
	logicalSceneXML.appendChild(modelItemsXML);
	logicalSceneXML.appendChild(dataItemsXML);
	logicalSceneXML.appendChild(conditionItemsXML);
	logicalSceneXML.appendChild(operationsXML);
	logicalSceneXML.appendChild(dependenciesXML);
	console.log(logicalSceneXML);

	var logicalSceneXMLStr=mxUtils.getXml(logicalSceneXML);
	return logicalSceneXMLStr;
}

/**
 * Set mxgraph list from XML in type string.
 */
Graph.prototype.setMxgraphList=function(Str){
	if(Str!=""){
		var doc =mxUtils.parseXml(Str).children[0];
		var mxgraphList=[];
		for(let i=0;i<doc.children.length;i++){
			let mxgraph={};
			let uid=doc.children[i].getAttribute("uid");
			mxgraph.uid=uid;
			let graphXML=doc.children[i].getAttribute("graphXML");
			mxgraph.graphXML=graphXML;
			mxgraphList.push(mxgraph);
		}
		this.mxgraphList=mxgraphList;
		// console.log(mxgraphList);
	}
}

/**
 * Set logical scene list from XML in type string.
 */
Graph.prototype.setLogicalScene=function(Str){
	if(Str!=""){
		var modelItems=[];
