...
var OpenNewFile = function(editorUi)
{
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
		...
		graph.setMxgraphList(value);
		for(let i=0;i<graph.mxgraphList.length;i++){
			if(graph.mxgraphList[i].uid=="basicXML"){
...
var OpenNewFile2 = function(editorUi)
{
	var div = document.createElement("div");
    var input = document.createElement("input");
	input.type = "file";
...
/**
 * Constructs a new read dialog
 */
var LogicalTasksReadDialog=function(editorUi){
    ...
    head.innerHTML='Load Tasks';
    var tasks=document.createElement('div');
    tasks.setAttribute('id','tasksContent');
    tasks.style.height='450px';
    tasks.style.padding='10px';
    tasks.style.marginBottom='30px';
    tasks.style.boxSizing = 'border-box';
    tasks.style.border = 'solid 1px #d5d5d5';
    tasks.style.borderRadius = '4px';
    tasks.style.overflowY='scroll';

    dialogContent.appendChild(head);
    dialogContent.appendChild(tasks);

	var taskData;
	var reg=/groupID=(\S*)/;
	var url=window.location.href;
	var groupID=url.match(reg)[1].replace("ComputableModel","LogicalModel");
    var dataJSON=new Object();
    dataJSON["Model"]="Logical";
    dataJSON["Type"]="Read";
    dataJSON["CID"]=groupID;
    $.ajax({
        type: "POST",
        url: "/TeamWorking/MxGraphServlet",
        data: dataJSON,
        success: function (data) {
            var str="";
            var jsonArray=eval(data);//字符串转数组
            taskData=jsonArray;
			if(jsonArray.length<1){
				...
				for(var i=0;i<jsonArray.length;i++) {
					str+='<div style="height:150px;width:400px;border:solid 1px #d5d5d5;margin:20px;">\n' +
						...
						'\t\t<span style="font-weight:bold">'+jsonArray[i]["TaskName"]+'</span>\n' +
						'\t\t<span class="deleteTask" style="float:right;margin:5px;cursor:pointer;" data-projectid="'+jsonArray[i]["ProjectID"]+'" onclick="deleteTask(this)">X</span>\n'+
						'\t\t<div style="width:350px;height:100px;">\n' +
						'\t\t<p style="text-indent:2em;margin:0px;">'+jsonArray[i]["Description"]+'</p>\n' +
						'\t\t</div>\n' +
						'\t\t<sub style="float:right;margin-right:15px;">'+jsonArray[i]["CreateTime"]+'</sub>\n' +
						'\t</div>';
				}	
			}
        ...
        error: function () {
            // var str='<div style="height:150px;width:400px;border:solid 1px #d5d5d5;margin:20px;">\n' +
            //    '\t\t<input type="radio" name="tasks" id="task" value="xml">\n' +
            //    '\t\t<span style="font-weight:bold">name</span>\n' +
            //    '\t\t<div style="width:350px;height:100px;">\n' +
            //    '\t\t<p style="text-indent:2em;margin:0px;">description</p>\n' +
            //    '\t\t</div>\n' +
            //    '\t\t<sub style="float:right;margin-right:15px;">create time</sub>\n' +
            //    '\t</div>';
            // $('#tasksContent').html(str);
            alert("Read fail!");
        }});
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
        if ($('input[name="tasks"]:checked').val()){
            window.taskID=taskData[$('input[name="tasks"]:checked').val()]["ProjectID"];

            window.taskName=taskData[$('input[name="tasks"]:checked').val()]["TaskName"];
            window.taskDescription=taskData[$('input[name="tasks"]:checked').val()]["Description"];
			var graph = editorUi.editor.graph;
			graph.setMxgraphList(taskData[$('input[name="tasks"]:checked').val()]["GraphXML"]);
			graph.setLogicalScene(taskData[$('input[name="tasks"]:checked').val()]["LogicalXML"]);

            document.getElementById("graphName").value=taskData[$('input[name="tasks"]:checked').val()]["TaskName"];
...
var SetDataServiceDialogOld = function(editorUi,title,uid)
{
	var graph=editorUi.editor.graph;
	var dataInstances=graph.computationalScene.dataInstances;
	var exist=0;var index;
	for(var i=0;i<dataInstances.length;i++){
		if(dataInstances[i].uid==uid){
			index=i;exist=1;
			break;
		}
	}
    ...
    var BaseUrlPanel = document.createElement("div");
    BaseUrlPanel.style.textAlign = "left";
	BaseUrlPanel.style.minWidth="430px";
    var p = document.createElement("span");
    p.innerHTML = "Base Url:";
    p.style.fontWeight = "bold";
    BaseUrlPanel.appendChild(p);

    var input = document.createElement("input");
    input.style.width = "300px";
	input.style.margin="10px 0 10px 10px";
	input.id="dataURLText";
	if(exist){
		input.value=dataInstances[index].url;
	}
    BaseUrlPanel.appendChild(input);

	var buttonAdd=document.createElement("button");
	buttonAdd.innerHTML="Select";
	buttonAdd.style.height="20px";
	buttonAdd.style.margin="10px 10px";
	buttonAdd.style.float="initial";
	buttonAdd.onclick=function(){
		confirm("add data");
	}
    BaseUrlPanel.appendChild(buttonAdd);

    div.appendChild(BaseUrlPanel);

    var title2Panel = document.createElement("div");
	title2Panel.style.textAlign = "left";
	title2Panel.style.minWidth="200px";
    var p2 = document.createElement("span");
	p2.innerHTML = "Request Parameter:";
    title2Panel.appendChild(p2);

    var buttonAddParamter = document.createElement("button");
    buttonAddParamter.innerHTML = "+";
    buttonAddParamter.style.height="20px";
    buttonAddParamter.style.width="20px";
	buttonAddParamter.style.margin="10px 20px";
	buttonAddParamter.style.float="initial";
	buttonAddParamter.style.padding="0";
    buttonAddParamter.onclick = function () {
		var tr=document.createElement("tr");
		var tdKey=document.createElement("td");
		tdKey.style.width="45%";
		var inputKey=document.createElement("input");
		inputKey.type="text";
		inputKey.className="dataKey";
		tdKey.appendChild(inputKey);
		tr.appendChild(tdKey);
		var tdValue=document.createElement("td");
		tdValue.style.width="45%";
		var inputValue=document.createElement("input");
		inputValue.type="text";
		inputValue.className="dataValue";
		tdValue.appendChild(inputValue);
		tr.appendChild(tdValue);
		var tdDelete=document.createElement("td");
		tdDelete.style.width="10%";
    ...
    title2Panel.appendChild(buttonAddParamter);

    div.appendChild(title2Panel);
    ...
    theadTr.innerHTML = "</th><th width='45%'>Key</th><th width='45%'>Value</th><th width='10%'>Delete</th>"
    thead.appendChild(theadTr);
    table.appendChild(thead);

	var tbody = document.createElement("tbody");
	tbody.id="dataServiceTable";
	tbody.style.minHeight="130px";
	tbody.style.overflowY="auto";
	if(exist){
		for(var i=0;i<dataInstances[index].parameters.length;i++){
			var tr=document.createElement("tr");
			var tdKey=document.createElement("td");
			tdKey.style.width="45%";
			var inputKey=document.createElement("input");
			inputKey.type="text";
			inputKey.className="dataKey";
			inputKey.value=dataInstances[index].parameters[i].key;
			tdKey.appendChild(inputKey);
			tr.appendChild(tdKey);
			var tdValue=document.createElement("td");
			tdValue.style.width="45%";
			...
			inputValue.type="text";
			inputValue.className="dataValue";
			inputValue.value=dataInstances[index].parameters[i].value;
			tdValue.appendChild(inputValue);
			tr.appendChild(tdValue);
			var tdDelete=document.createElement("td");
			tdDelete.style.width="10%";
	...
	tableDiv.style.minHeight="150px";
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
		...
		for(var i=0;i<tbody.childElementCount;i++){
			var parameter={};
			parameter.key=dataKeys[i].value;
			parameter.value=dataValues[i].value;
			parameters.push(parameter);
		}
		if(exist){
			graph.computationalScene.dataInstances[index].uid=uid;
			graph.computationalScene.dataInstances[index].url=$("#dataURLText").val();
			graph.computationalScene.dataInstances[index].parameters=parameters;
		}else{
			var dataInstance={};
			dataInstance.uid=uid;
			dataInstance.url=$("#dataURLText").val();
			dataInstance.parameters=parameters;
			graph.computationalScene.dataInstances.push(dataInstance);
		}
		console.log(graph.computationalScene.dataInstances);
        editorUi.hideDialog();
    });
...
var SelectDataServiceDialog=function(editorUi,elt){
	var div =document.createElement("div");
	var dataListDiv=document.createElement("div");
	dataListDiv.style.minHeight="160px";
	dataListDiv.style.overflowY="scoll";
	var dataListHtml="";
	for(var i=0;i<graph.computationalScene.dataInstances.length;i++){
		dataListHtml+='<div style="width:100%";border:solid 1px #d5d5d5;margin-top:5px;><input type="radio" name="datas"><span style="font-weight:bold">'+
		graph.computationalScene.dataInstances[i].dataId+'</span><div><span>'+graph.computationalScene.dataInstances[i].description+'</span></div></div>';
	}
	dataListDiv.innerHTML=dataListHtml;
	div.appendChild(dataListDiv);
...
var SetDataServiceDialog = function(editorUi,title,uid)
{
	var graph=editorUi.editor.graph;
	var dataInstances=graph.computationalScene.dataInstances;
	var exist=0;var index;
	...
	buttonAdd.style.float="initial";
	buttonAdd.onclick=function(){
		editorUi.showDialog(new SelectDataServiceDialog(editorUi,this).container,200,200,true,false);
	}
    BaseUrlPanel.appendChild(buttonAdd);
    ...
    var thead = document.createElement("thead");
    var theadTr = document.createElement("tr");
    theadTr.innerHTML = "</th><th width='50%'>Key</th><th width='50%'>Value</th>"
    thead.appendChild(theadTr);
    table.appendChild(thead);
	...
	inputValue.className="dataValue";
	if(exist){
		inputValue.value=dataInstances[index].dataId;
	}
	tdValue.appendChild(inputValue);
	tr.appendChild(tdValue);
	tbody.appendChild(tr);
    ...
    var dataDescription = document.createElement('textarea');
	dataDescription.setAttribute('type', 'text');
	dataDescription.setAttribute('id', 'dataDescription');
	if(exist){
		dataDescription.value=dataInstances[index].description;
	}
    dataDescription.style.fontSize = '12px';
    dataDescription.style.fontFamily = 'Arial';
    dataDescription.style.overflow = 'hidden';
    dataDescription.style.boxSizing = 'border-box';
    dataDescription.style.border = 'solid 1px #d5d5d5';
    dataDescription.style.borderRadius = '4px';
    dataDescription.style.width = '100%';
    dataDescription.style.outline = 'none';
    dataDescription.style.padding = '6px';
    dataDescription.style.marginTop = '5px';
    dataDescription.style.marginBottom='20px';
    dataDescription.rows = 15;
    dataDescription.onclick = function(){
        this.focus();
	};
    ...
    var okBtn = mxUtils.button(mxResources.get('ok'), function()
    {
		if(exist){
			graph.computationalScene.dataInstances[index].uid=uid;
			graph.computationalScene.dataInstances[index].url=$("#dataURLText").val();
			graph.computationalScene.dataInstances[index].dataId=$(".dataValue").val();
			graph.computationalScene.dataInstances[index].description=$("#dataDescription").val();
		}else{
			var dataInstance={};
			dataInstance.uid=uid;
			dataInstance.url=$("#dataURLText").val();
			dataInstance.dataId=$(".dataValue").val();
			dataInstance.description=$("#dataDescription").val();
			graph.computationalScene.dataInstances.push(dataInstance);
		}
		// console.log(graph.computationalScene.dataInstances);
        editorUi.hideDialog();
...
var SetModelServiceDialog = function(editorUi,title,uid)
{
	var graph=editorUi.editor.graph;
	var modelInstances=graph.computationalScene.modelInstances;
	var exist=0;var index;
	for(var i=0;i<modelInstances.length;i++){
		if(modelInstances[i].uid==uid){
			index=i;exist=1;
			$.ajax({
				url:modelInstances[i].request.url,
				success:function(data){
					var modelData=JSON.parse(data);
					modelStates=modelData.mdl.ModelClass.Behavior.StateGroup.States.State;
				},
				error:function(){
					alert("please confirm your url!");
				}
			});
			break;
		}
    ...
    input.style.width = "300px";
	input.style.margin="10px 0 10px 10px";
	input.id="modelURLText";
	if(exist){
		input.value=modelInstances[index].request.url;
	}
    BaseUrlPanel.appendChild(input);

	var buttonAdd=document.createElement("button");
	buttonAdd.innerHTML="submit";
	buttonAdd.style.height="20px";
	buttonAdd.style.margin="10px 10px";
	buttonAdd.style.float="initial";
	buttonAdd.onclick=function(){
		statesDiv.innerHTML="";
		$.ajax({
			url:$("#modelURLText").val(),
			// url:"http://172.21.212.7:8060/modelser/mdl/xml/5c18b3390e7c2d4764335a7d",
			success:function(data){
				var modelData=JSON.parse(data);
				modelStates=modelData.mdl.ModelClass.Behavior.StateGroup.States.State;
				console.log(modelStates);
				console.log();
				if(modelStates.length===undefined){
					addState(modelStates);
				}
				else{
					for(var i=0;i<modelStates.length;i++){
						addState(modelStates[i]);
					}
				}
			},
			error:function(){
		...
		function addState(modelState){
			var stateTitleDiv=document.createElement("div");
			var stateTile=document.createElement("span");
			stateTile.innerHTML="State:"
			stateTile.style.marginRight="10px";
			stateTitleDiv.appendChild(stateTile);
			var stateName=document.createElement("span");
			stateName.innerHTML=modelState.$.name;//stateName
			stateTitleDiv.appendChild(stateName);
			var eventsTitle=document.createElement("span");
			eventsTitle.innerHTML="Events:";
			eventsTitle.style.marginLeft="30px";
			stateTitleDiv.appendChild(eventsTitle);
			statesDiv.appendChild(stateTitleDiv);
			...
			var thead = document.createElement("thead");
			var theadTr = document.createElement("tr");
			theadTr.innerHTML = "</th><th width='20%'>Name</th><th width='20%'>Type</th><th width='60%'>DataId</th>"
			thead.appendChild(theadTr);
			table.appendChild(thead);
			var tbody = document.createElement("tbody");
			tbody.id="dataServiceTable";
			tbody.style.minHeight="130px";
			tbody.style.overflowY="auto";
			for(var j=0;j<modelState.Event.length;j++){
				var tr=document.createElement("tr");
				...
				eventName.value=modelState.Event[j].$.name;//eventName
				eventName.disabled=true;
				tdName.appendChild(eventName);
				tr.appendChild(tdName);
				var tdType=document.createElement("td");
				tdType.style.width="20%";
				var eventType=document.createElement("input");
				eventType.type="text";
				eventType.className="eventType";
				eventType.value=modelState.Event[j].$.type;//eventType
				eventType.disabled=true;
				tdType.appendChild(eventType);
				tr.appendChild(tdType);
				var tdDataId=document.createElement("td");
				tdDataId.style.width="60%";
				var dataIdText=document.createElement("input");
				dataIdText.type="text";
				dataIdText.className="modelDataId";
				dataIdText.style.width="88%";
				tdDataId.appendChild(dataIdText);
				var dataIdSelect=document.createElement("button");
				dataIdSelect.style.padding="0";
				dataIdSelect.innerHTML="+";
				dataIdSelect.style.width="20px";
				dataIdSelect.style.height="20px";
				dataIdSelect.style.float="right";
				dataIdSelect.onclick=function(){
					// confirm("add data");//
					editorUi.showDialog(new SelectDataServiceDialog(editorUi,this).container,500,500,true,false);
				};
				tdDataId.appendChild(dataIdSelect);
				tr.appendChild(tdDataId);
				tbody.appendChild(tr);
			}
			table.appendChild(tbody);
			statesDiv.appendChild(table);
			$("#modelStatesDiv").append(statesDiv);
		}
	}
    BaseUrlPanel.appendChild(buttonAdd);
    ...
    var thead = document.createElement("thead");
    var theadTr = document.createElement("tr");
    theadTr.innerHTML = "</th><th width='20%'>Key</th><th width='20%'>Value</th><th width='60%'>Description</th>"
    thead.appendChild(theadTr);
    table.appendChild(thead);
	...
	tbody.style.minHeight="130px";
	tbody.style.overflowY="auto";
	var requestList=[{"keyClass":"modelId","keyValue":"Id","valueClass":"modelIdValue","descClass":"idDesc"},
	{"keyClass":"userToken","keyValue":"UserToken","valueClass":"userTokenValue","descClass":"userDesc"}];
	for(var i=0;i<requestList.length;i++){
		var tr=document.createElement("tr");
		var tdKey=document.createElement("td");
		tdKey.style.width="20%";
		var inputKey=document.createElement("input");
		inputKey.type="text";
		inputKey.className=requestList[i].keyClass;
		inputKey.value=requestList[i].keyValue;
		inputKey.disabled=true;
		tdKey.appendChild(inputKey);
		tr.appendChild(tdKey);
		var tdValue=document.createElement("td");
		tdValue.style.width="20%";
		var inputValue=document.createElement("input");
		inputValue.type="text";
		inputValue.className=requestList[i].valueClass;
		if(exist){
			inputValue.value=modelInstances[index].request[requestList[i].keyClass];
		}
		tdValue.appendChild(inputValue);
		tr.appendChild(tdValue);
		var tdDesc=document.createElement("td");
		tdDesc.style.width="60%";
		var descArea=document.createElement("input");
		descArea.type="text";
		descArea.className=requestList[i].descClass;
		descArea.style.width="98%";
		if(exist){
			descArea.value=modelInstances[index].request[requestList[i].descClass];
		}
	...
	statesDiv.style.marginBottom="20px";
	statesDiv.style. minHeight="250px";
	statesDiv.id="modelStatesDiv"
	if(exist){
		var states=graph.computationalScene.modelInstances[index].states;
		for(var i=0;i<states.length;i++){
			...
			stateTitleDiv.appendChild(stateTile);
			var stateName=document.createElement("span");
			stateName.innerHTML=states[i].name;//stateName
			stateTitleDiv.appendChild(stateName);
			var eventsTitle=document.createElement("span");
			eventsTitle.innerHTML="Events:";
			eventsTitle.style.marginLeft="30px";
			...
			for(var j=0;j<states[i].events.length;j++){
				var tr=document.createElement("tr");
				var tdName=document.createElement("td");
				tdName.style.width="20%";
				var eventName=document.createElement("input");
				eventName.type="text";
				eventName.className="eventName";
				eventName.value=states[i].events[j].name;//eventName
				eventName.disabled=true;
				tdName.appendChild(eventName);
				tr.appendChild(tdName);
				var tdType=document.createElement("td");
				...
				eventType.type="text";
				eventType.className="eventType";
				eventType.value=states[i].events[j].type;//eventType
				eventType.disabled=true;
				tdType.appendChild(eventType);
				tr.appendChild(tdType);
				var tdDataId=document.createElement("td");
				...
				dataIdText.className="modelDataId";
				dataIdText.style.width="88%";
				dataIdText.value=states[i].events[j].dataId;
				tdDataId.appendChild(dataIdText);
				var dataIdSelect=document.createElement("button");
				dataIdSelect.style.padding="0";
				dataIdSelect.innerHTML="+";
    ...
    {
		var request={};
		request.url=$("#modelURLText").val();
		request.modelId=$(".modelIdValue").val();
		request.idDesc=$(".idDesc").val();
		request.userToken=$(".userTokenValue").val();
		request.userDesc=$(".userDesc").val();

		var eventNodes=[];
		$(".modelDataId").each(function(){
			eventNodes.push($(this).val());
		});
		var eventsList=[];
		if(modelStates.length===undefined){
			var events=[];
			for(var i=0;i<eventNodes.length;i++){
				events.push(eventNodes[i]);
			}
			...
			for(var i=0;i<modelStates.length;i++){
				var events=[];
				for(var j=0;j<modelStates[i].Event.length;j++){
					events.push(eventNodes[count++]);
				}
				eventsList.push(events);
			}
		}
		...
		var states=[];
		if(modelStates.length===undefined){
			saveState(modelStates,eventsList[0]);
		}
		else{
			for(var i=0;i<modelStates.length;i++){
				saveState(modelStates[i],eventsList[i]);
			}
		}
		function saveState(modelState,modelEvents){
			var state={};
			state.id=modelState.$.id;
			state.name=modelState.$.name;
			var events=[];
			for(var i=0;i<modelState.Event.length;i++){
				var event={};
				// event.id=
				event.name=modelState.Event[i].$.name;
				event.type=modelState.Event[i].$.type;
				event.dataId=modelEvents[i];
				events.push(event);
			}
			state.events=events;
			states.push(state);
		...
		if(exist){
			graph.computationalScene.modelInstances[index].uid=uid;
			graph.computationalScene.modelInstances[index].request=request;
			graph.computationalScene.modelInstances[index].states=states;
		}else{
			var modelInstance={};
			modelInstance.uid=uid;
			modelInstance.request=request;
			modelInstance.states=states;
			graph.computationalScene.modelInstances.push(modelInstance);
		}
		console.log(graph.computationalScene.modelInstances);
        editorUi.hideDialog();
    });
    okBtn.className = 'geBtn';
    div.appendChild(okBtn);
